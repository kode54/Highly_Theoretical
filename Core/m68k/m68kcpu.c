...
static int default_tas_instr_callback(m68ki_cpu_core *cpu)
{
...
unsigned int m68k_get_reg(m68ki_cpu_core *cpu, m68k_register_t regnum)
{
  switch(regnum)
  {
...
#if M68K_EMULATE_FC == OPT_ON
void m68k_set_fc_callback(void  (*callback)(void *param, unsigned int new_fc))
{
 ...
 */
void m68k_update_irq(m68ki_cpu_core *cpu, unsigned int mask)
{
  /* Update IRQ level */
  CPU_INT_LEVEL |= (mask << 8);
  
#ifdef LOGVDP
  error("[%d(%d)][%d(%d)] IRQ Level = %d(0x%02x) (%x)\n", v_counter, cpu->cycles/3420, cpu->cycles, cpu->cycles%3420,CPU_INT_LEVEL>>8,FLAG_INT_MASK,m68k_get_reg(M68K_REG_PC));
#endif
...
void m68k_set_irq_delay(m68ki_cpu_core *cpu, unsigned int int_level)
{
  /* Prevent reentrance */
  if (!cpu->irq_latency)
  {
    ...
    if ((REG_IR & 0xF000) != 0x2000)
    {
      ...
      cpu->irq_latency = 1;
      m68ki_trace_t1(cpu) /* auto-disable (see m68kcpu.h) */
      m68ki_use_data_space(cpu) /* auto-disable (see m68kcpu.h) */
      REG_IR = m68ki_read_imm_16(cpu);
      m68ki_instruction_jump_table[REG_IR](cpu);
      m68ki_exception_if_trace(cpu) /* auto-disable (see m68kcpu.h) */
      cpu->irq_latency = 0;
    }
...
void m68k_run(m68ki_cpu_core *cpu, unsigned int cycles) 
{
  ...
  m68ki_set_address_error_trap(cpu) /* auto-disable (see m68kcpu.h) */

  /* Save end cycles count for when CPU is stopped */
  cpu->cycle_end = cycles;

#ifdef LOGVDP
  error("[%d][%d] m68k run to %d cycles (%x)\n", v_counter, cpu->cycles, cycles, cpu->pc);
#endif
  ...
  while (cpu->cycles < cycles)
  {
    ...
    /* Execute instruction */
	  m68ki_instruction_jump_table[REG_IR](cpu); /* TODO: use labels table with goto */
...
void m68k_init(m68ki_cpu_core *cpu)
{
#ifdef BUILD_TABLES
...
void m68k_clear_halt(m68ki_cpu_core *cpu)
{
